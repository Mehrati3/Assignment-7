...
static string BuildForm(PageInput input, string path, AntiforgeryTokenSet antiForgery, ModelStateDictionary? modelState = null)
{
    ...
    string modifiedContent = input.Content;

    if (input.Name.Equals("emoji", StringComparison.OrdinalIgnoreCase))
    {
        modifiedContent += " 😊"; // Add a smiley face next to the existing content if page name is "emoji"
    }

    var contentField = Div
        .Append(Label.Class("uk-form-label").Append(nameof(input.Content)))
        .Append(Div.Class("uk-form-controls")
            .Append(Textarea.Name("Content").Class("uk-textarea").Append(modifiedContent))
        );
    ...
    var submit = Div.Style("margin-top", "20px").Append(Button.Class("uk-button uk-button-primary")
        //.Attribute("onclick", "insertEmoji(event)")
        .Append("Submit"));

    //var script = Script.Append(@"
    ...
    //        var emoji = emojiInput.value; // Get the emoji from the emoji input field

    //        textarea.value = textBefore + ' ' + emoji + ' ' + textAfter; // Add spaces around the emoji
    //        textarea.focus();
    //        textarea.setSelectionRange(cursorPos + emoji.length + 2, cursorPos + emoji.length + 2); // Set cursor position after the inserted emoji
    //    }
    //");
          ...
          <style>
            ...
            a:link { color: red; }
              ...
              .container {
                  ...
                  margin: 20px auto;
                  padding: 0 20px;
                  background-color: #ecf4ff; /* Light blue container */
                  border-radius: 5px;
              ...
              .sidebar {
                  background-color: #bbdefb; /* Light blue sidebar */
                  padding: 10px;
                  border-radius: 5px;
                  //margin-left : 20%;
              }
              ...
              .uk-navbar-container:not(.uk-navbar-transparent) {
                background-color: #1e88e5; /* Dark blue navbar container */
              }
             ...
             .page-content {
              margin-bottom: 20px; /* Add margin at the bottom of the content */
            ` }
          </style>
          """),
